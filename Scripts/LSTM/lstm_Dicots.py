...
# Filter orthogroups with >200 sequences in training split only
...
for epoch in range(5):
    ...
    if dev_metrics['f1'] > best_f1:
        ...
        torch.save(model.state_dict(), "/Model/LSTM/LSTM_Dicots.pt")
...
model.load_state_dict(torch.load("/Model/LSTM/LSTM_Dicots.pt"))
model.eval()
...
for name, df in test_paths.items():
    ...
    save_classification_report(labels, preds, f"/Tables/LSTM/classification_report_{name}.csv")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("/Figures/LSTM/accuracy_plot_Dicots.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("/Figures/LSTM/loss_plot_Dicots.png")