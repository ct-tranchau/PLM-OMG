...
# Filter orthogroups with >100 samples in training split only
train_ogs = train_split['Orthogroup'].value_counts()
valid_ogs = train_ogs[train_ogs > 100].index
...
best_dev_f1 = 0.0
best_model_path = "/Model/PROBERT/proteinbert_Monocots_5.pt"
...
for name, df in test_sets.items():
    ...
    save_classification_report(labels, preds, f"/Tables/PROBERT/classification_report_test_monocots_5_{name}.csv")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("/Figures/PROBERT/accuracy_plot_Monocots_5.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("/Figures/PROBERT/loss_plot_Monocots_5.png")